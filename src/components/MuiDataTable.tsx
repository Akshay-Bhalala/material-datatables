...
import {
  ...
  MuiDataTableRow,
  MuiDataTableColumn,
  MuiDataTableSortModel,
  MuiDataTableFilterModel,
  MuiDataTableToolbarAction,
} from "../types";
...
const MuiDataTable: React.FC<MuiDataTablePropsWithTitle> = (
  props: MuiDataTablePropsWithTitle
) => {
  const {
    columns,
    rows,
    ...
    toolbarActions = [], // Default to empty array for minimal toolbar
    showSearch = true,
    showDownload = true,
    sx,
    className,
    ...
    virtualization,
    rowHeight,
    tableColor = "#ffffff", // Default to light theme
    tableTextColor = "#212121", // Default to dark text for light theme
    ariaLabel,
    title = "User List", // Default title from screenshot
  } = props;
  ...
  const pageSize = isPaginationControlled
    ? pagination!.pageSize
    : internalPageSize;
  ...
  const [internalSortModel, setInternalSortModel] = useState<
    MuiDataTableSortModel | undefined
  >(undefined);
  const sortModel: MuiDataTableSortModel | undefined = isSortControlled
    ? controlledSortModel
    : internalSortModel;
  ...
  const [internalFilterModel, setInternalFilterModel] =
    useState<MuiDataTableFilterModel>({});
  const filterModel: MuiDataTableFilterModel = isFilterControlled
    ? controlledFilterModel!
    : internalFilterModel;

  // Column visibility state (controlled/uncontrolled)
  const isColumnVisibilityControlled =
    !!controlledColumnVisibilityModel && !!onColumnVisibilityChange;
  const defaultVisibility = Object.fromEntries(
    columns.map((col) => [col.field, col.visible !== false])
  );
  ...
  const visibleColumns = columns.filter(
    (col: MuiDataTableColumn) => columnVisibilityModel[col.field] !== false
  );
  ...
  const getRowId = (row: MuiDataTableRow, rowIndex: number) =>
    row.id ?? rowIndex;
  ...
  const sortedRows = useMemo(() => {
    ...
    const col = columns.find(
      (c: MuiDataTableColumn) => c.field === sortModel.field
    );
    if (!col) return filteredRows;
    const sorted = [...filteredRows].sort(
      (a: MuiDataTableRow, b: MuiDataTableRow) => {
  ...
  const allPageRowsSelected =
    ...
    paginatedRows.every((row, i) => selectedRowIds.has(getRowId(row, i)));

  return (
    <div className={styles["mui-datatable-outer"]}>
      <div className={styles["mui-datatable-card"]}>
        <div className={styles["mui-datatable-title"]}>{title}</div>
        <div className={styles["mui-datatable-toolbar"]}>
          <div className={styles["mui-datatable-toolbar-right"]}>
            {showDownload && (
              <button
                className={styles["mui-datatable-download"]}
              ...
              <div className={styles["mui-datatable-actions"]}>
                {toolbarActions.map((action, idx) => (
                  <button
                    ...
                    disabled={loading}
                    title={action.label}
                    className={styles["mui-datatable-action-btn"]}
                  >
                    {action.icon}
                  </button>
                ))}
              ...
              <tr>
                ...
                {visibleColumns.map((col: MuiDataTableColumn) => (
                  <th
                    ...
                    onClick={() =>
                      !loading && col.sortable && handleSort(col.field)
                    }
                  >
                    {col.headerName}
                    {col.sortable && (
              ...
              {loading ? (
                <tr>
                  <td
                    ...
                    className={styles["mui-datatable-loading-cell"]}
                  >
                    <span>Loading...</span>
                  </td>
                </tr>
                ...
                paginatedRows.map((row: MuiDataTableRow, rowIndex: number) => {
                  ...
                  return (
                    <tr
                      ...
                      className={
                        isSelected ? styles["mui-datatable-row-selected"] : ""
                      }
